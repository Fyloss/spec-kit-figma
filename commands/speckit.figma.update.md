...
4. **Check for features orphaned by the 2.0.0 trigger change.** An update is the
   only moment anything looks at the *existing* features, and there is exactly one
   failure mode that produces no signal of its own:

   ```bash
   ./.specify/extensions/figma/scripts/bash/figma-check-orphans.sh
   ```

   ```powershell
   ./.specify/extensions/figma/scripts/powershell/figma-check-orphans.ps1
   ```

   Before 2.0.0 a mapped `pageToPackageMapping` was enough to trigger design
   context, so a spec generated by 1.6.x can carry a full design section that
   records **no url** ("None — context derived from page mapping."). Under 2.0.0
   the link is the trigger, and the recovery path parses urls out of the committed
   `spec.md` — with none to find, the run falls through to `no-figma-link`, under
   which you are told to say nothing about Figma at all. So `/speckit.plan` and
   `/speckit.tasks` silently stop adding the design context that `spec.md` still
   displays, and nothing anywhere reports it.

   When `orphans` is non-empty, **list the affected features in your reply** with
   the two ways out: paste the Figma link of the frame each one targets into its
   `spec.md` (inside the design section), or enable `autoIntrospect` on the
   target so the mapping becomes a legitimate trigger again. **Do not edit any
   spec yourself** — which frame a feature targeted is precisely what those specs
   failed to record, and guessing it is how the wrong creative gets implemented.
   An empty `orphans` list is reported as nothing at all.
