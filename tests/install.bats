...
@test "install appends the auto-context hook to existing speckit command prompts" {
...
@test "re-running install keeps a single copy of the auto-context hook" {
...
@test "install notes when no speckit command files exist to hook" {
  run "$INSTALL" --target "$WORKSPACE"
  [ "$status" -eq 0 ]
