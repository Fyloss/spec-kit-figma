...
#
# -IdentifiedOnly drops the last-resort single-candidate rule, so ONLY a document
# the current feature positively owns is returned. A caller that READS design
# context out of the document needs that: with nothing identifying the feature,
# "the only spec around" is some OTHER feature's, and inheriting its creative
